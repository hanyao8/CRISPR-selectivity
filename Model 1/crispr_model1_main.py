...
k_B=1.38e-23
T=2000 #(High temp. approximation?)
beta=1/k_B/T
...
ts=["A","T","A","A","A","C","G","A","C","T","G","G","C","T","A","A","G","C","T","A"]
...
cs=[] #Creating the complementary sequence of the user defined targeting sequence
for b in range(0,ts_len):
    if ts[b]=='A':
        cs.append('T')
    if ts[b]=='T':
        cs.append('A')
    ...
    if ts[b]=='G':
        cs.append('C')
    if ts[b]!='A' and  ts[b]!='T' and ts[b]!='C' and ts[b]!='G':
        raise Exception("Invalid Nucleotide")


"""Calculating the probability of the targeted sequence being a complementary sequence, 
given the relative nucleotide frquencies
"""
p_cs=1  #Intermediate array to calculate prob. of cs.
for c in range(0,ts_len):
    if ts[c]=='A':
        p_cs*=p_A
    if ts[c]=='T':
        p_cs*=p_T
