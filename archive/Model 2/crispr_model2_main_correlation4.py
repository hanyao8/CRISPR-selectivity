...
for n_ncp in range(0,ts_len+1):
    #Z_cml_list.append(0)
    #if n_ncp>2:
     #   if round(Z_cml_list[n_ncp-1],-int(math.floor (math.log10(Z_cml_list[n_ncp-1])) )+1-sf ) == round(Z_cml_list[n_ncp-2],-int(math.floor (math.log10(Z_cml_list[n_ncp-2])) )+1-sf ):
      #      raise Exception()
    print Z
    n_cp=ts_len-n_ncp
                                                    ...
                                                    for perm_T in next_permutation(seq_T):
                                                        for perm_C in next_permutation(seq_C):
                                                            for perm_G in next_permutation(seq_G):
                                                                ...
                                                                #Z_cml_list[n_ncp]+=P*np.exp(-beta*n_cp*BE)
