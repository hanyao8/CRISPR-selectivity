...
ts=["C","C","C","C","C","C","C","C","C","C","C","C","C","C","C","C","C","C","C","C"]
...
for n_cp in range(0,ts_len+1):
    ...
    for i in range(i_start,i_end+1): #Number of A nucleotides in targeted sequence
        ...
        for j in range(j_start,j_end+1): #Number of T nucleotides in targeted sequence
            ...
            if k_start<0: #Need more thought here
                k_start=0
            if n_ncp-i-j-k_start>0:
                k_start=n_ncp-i-j
                    ...
                    for n_ncp_A in range(n_ncp_A_start,n_ncp_A_end+1):
                        ...
                        for n_ncp_G in range(n_ncp_G_start,n_ncp_G_end+1):
                            ...
                            if n_ncp_A+n_ncp_T+n_ncp_C+n_ncp_G==n_ncp: #It is actually possible from the above that the sum does not add up to the total number of non-complementary pairs
                                ...
                                #a,b,c,d correspond to the number of A,T,C,G nucleotides on the targeted sequence that are attached to the complementary NT, violating the complementary pair number n_cp
                                if n_ncp_T>0:
                                    for a in range(1,min(n_ncp_T,i-(n_T-n_ncp_T))+1):
                                    ...
                                    omega+=0
                                if n_ncp_A>0:
                                    for b in range(1,min(n_ncp_A,j-(n_A-n_ncp_A))+1):
                                    ...
                                    omega+=0
                                if n_ncp_G>0:
                                    for c in range(1,min(n_ncp_G,k-(n_G-n_ncp_G))+1):
                                    ...
                                    omega+=0
                                if n_ncp_C>0:
                                    for d in range(1,min(n_ncp_C,l-(n_C-n_ncp_C))+1):
