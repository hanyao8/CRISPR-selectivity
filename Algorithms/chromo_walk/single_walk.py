...
T=310
beta=1/k_B/T
BE=-0.054*(1.6e-19) #Average binding energy corresponding to the below sequence used, derived from Santa Lucia 1998
...
class sim:
    ...
    def __init__(self,data,site0,PAM,ts_len):
        self.__data=data
        self.__site0=site0
        self.__PAM=PAM        
    ...
    #for j in range(site1,site1+bindings):
        for j in range(0,self.__data_len-self.__ts_len+1):
            ...
            if tgds.count('N')==0:
                ...
                self.__Z+=np.exp(-beta*G_binding)
                
                if tgds==cs:
                    self.__q_comp=np.exp(-beta*G_binding)
                    self.__comp_count+=1
